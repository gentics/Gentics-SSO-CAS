 ...
 * 
 * in addition it works together with the {@link ReauthenticationHelperFilter} - which is required to PUSH all allowed "reauth-tokens" into
 * this helper {@link #initForThread(List)} and at the end of the request retrieve all remaining "reauth-tokens" {@link #finishForThread()}
 * and store them back into the session.
 * 
 * @author herbert
 */
public class ReauthenticationHelper {
	...
	/**
	 * checks the given reauthId token and if it is found, remove it from the store. (returns true if a token exists.) (caller must make
	 * sure, that we have been initialized.)
	 * 
	 * @return true
	 */
	...
	/**
	 * asserts that the authentication helper was initialized by the {@link ReauthenticationHelperFilter} will throw a
	 * {@link IllegalStateException} if it was not yet initialized.
	 */
	 ...
	 *            authentication id
	 * @param redirectURL
	 *            URL to redirect if the user did not pass authentication
	 * @return true when the user passed the authentication or false if not
	 */
	public boolean reauthenticateOrRedirect(ResourceRequest request, ResourceResponse response, String id, String redirectURL) {
		assertInitialized();
		if (checkToken(id)) {
			return true;
		}
	 ...
	 * 
	 * @param id
	 *            the "instance id" which identifies the password reauthentication (usually the portlet entity name or portlet name.)
	 */
	public boolean reauthenticateOrShowPasswordField(PortletRequest request, RenderResponse response, String id, String name) {
		...
		try {
			showPasswordField(response.getWriter(), showError,
					successurl.toString() + "&" + REAUTH_ID_PARAMETER + "=" + URLEncoder.encode(id, "UTF-8"), errorurl.toString(), name);
		} catch (IOException e) {
	...
	private void showPasswordField(PrintWriter writer, boolean showError, String serviceUrl, String errorUrl, String name) {
		// we do it inline, because it wouldn't make much sense to use templates or whatever.. just keep HTML simple so it can be easily
		// customized
		// using CSS
